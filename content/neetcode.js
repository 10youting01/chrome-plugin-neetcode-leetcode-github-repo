...
function getCodeFromPageContext(slug) {
  return new Promise((resolve) => {
    ...
    function requestCode() {
      window.postMessage({ type: "NC_GITHUB_PUSHER_CODE_REQUEST", requestId, slug }, "*");
    }

    const existingBridge = document.documentElement?.querySelector?.("script[data-nc-github-pusher-bridge='true']");
    if (existingBridge) {
      requestCode();
      return;
    }
    ...
    script.dataset.ncGithubPusherBridge = "true";
    script.src = chrome.runtime.getURL("content/neetcode-page-bridge.js");
    script.onload = requestCode;
    script.onerror = () => {
      window.clearTimeout(timeout);
      window.removeEventListener("message", handleMessage);
      resolve({ code: "", language: "" });
